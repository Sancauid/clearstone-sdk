...
from clearstone.storage.sqlite import TraceStore, SpanBuffer
...
def create_mock_span(trace_id, name):
    return Span(trace_id=trace_id, name=name, start_time_ns=time.time_ns(), instrumentation_name="test", instrumentation_version="1.0")
...
def test_trace_store_initialization(trace_store, db_path):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='spans';")
...
def test_span_buffer_is_thread_safe(trace_store):
    ...
    with patch.object(trace_store, 'write_spans', wraps=trace_store.write_spans) as mock_write:

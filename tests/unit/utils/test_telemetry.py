...
from clearstone.utils.telemetry import (
    _TELEMETRY_DISABLED_ENV_VAR,
    TelemetryManager,
...
@patch("clearstone.utils.telemetry.TelemetryManager._send_event")
def test_integration_with_policy_engine(mock_send, clean_env):
    """Test that initializing a component like PolicyEngine triggers a telemetry event."""
    from clearstone.core.policy import Policy, PolicyEngine, reset_policies

    @Policy(name="dummy")
    def dummy_policy(context):
        return None
    ...
    mock_send.assert_called_once()
    call_args = mock_send.call_args[0][0]
    assert call_args["payload"] == {"name": "PolicyEngine"}

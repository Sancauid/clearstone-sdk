...
from clearstone.core.context import (
    PolicyContext, get_current_context, set_current_context,
    context_scope, create_context
)
...
async def test_context_is_async_safe():
    ...
    await asyncio.gather(
        task("user1", "agentA", 0.02),
        task("user2", "agentB", 0.01)
    )

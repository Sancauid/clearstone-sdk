...
from clearstone.observability.models import Trace, Span, SpanStatus, SpanKind
from clearstone.core.actions import ActionType

def mock_span(name, kind=SpanKind.INTERNAL, status=SpanStatus.OK, cost=0.0, tool_name=None, error_message=None):
  attributes = {}
  if cost > 0:
...
def test_assert_llm_cost_is_less_than():
  ...
  trace = Trace(
    trace_id="t1",
    spans=[
      mock_span("s1", cost=0.02),
      mock_span("s2", cost=0.02)
    ],
    root_span_id="",
    agent_id="",
    agent_version="",

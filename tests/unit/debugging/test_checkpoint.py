...
from clearstone.debugging.checkpoint import Checkpoint, CheckpointSerializer, CheckpointManager
from clearstone.observability.models import Trace, Span
...
def mock_trace():
  ...
  span1 = Span(trace_id="t1", span_id="s1", name="parent", start_time_ns=1, instrumentation_name="t", instrumentation_version="1")
  span2 = Span(trace_id="t1", span_id="s2", name="child", parent_span_id="s1", start_time_ns=2, instrumentation_name="t", instrumentation_version="1")
  return Trace(trace_id="t1", spans=[span1, span2], root_span_id="s1", agent_id="a1", agent_version="v1", environment="test", start_time_ns=1)
...
def test_checkpoint_serialization_roundtrip():
  ...
  span = Span(trace_id="t1", span_id="s1", name="test", start_time_ns=1, instrumentation_name="t", instrumentation_version="1")
  ...
  assert original_checkpoint.agent_state["memory"] == deserialized_checkpoint.agent_state["memory"]
  assert original_checkpoint.current_span.span_id == deserialized_checkpoint.current_span.span_id
...
def test_checkpoint_manager_create_and_save(checkpoint_manager, mock_trace):
  ...
  checkpoint_file = checkpoint_manager.checkpoint_dir / f"{checkpoint.trace_id}_{checkpoint.checkpoint_id}.ckpt"
...
def test_checkpoint_manager_load(checkpoint_manager, mock_trace):
  ...
  created_checkpoint = checkpoint_manager.create_checkpoint(agent, mock_trace, span_id="s2")
  filepath = checkpoint_manager.checkpoint_dir / f"{created_checkpoint.trace_id}_{created_checkpoint.checkpoint_id}.ckpt"

...
import pytest
from clearstone.core.policy import (
    Policy, PolicyEngine, reset_policies, get_policies
)
from clearstone.core.context import create_context, context_scope, set_current_context
from clearstone.core.actions import (
    ALLOW, BLOCK, PAUSE, ALERT, REDACT, ActionType
)
...
def test_policy_priority_sorting():
    ...
    def p0(context): return ALLOW
    
    @Policy(name="p100", priority=100)
    def p100(context): return ALLOW

    @Policy(name="p50", priority=50)
    def p50(context): return ALLOW
...
def test_engine_returns_first_non_allow_decision():
    ...
    assert decision.action == ActionType.PAUSE, "Highest priority action (PAUSE) should be returned."

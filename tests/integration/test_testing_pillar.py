...
def simple_research_agent(query: str, tracer):
  ...
  with tracer.span("research_workflow"):
    ...
    with tracer.span("search_tool", kind=SpanKind.CLIENT, attributes={"tool.name": "web_search"}):
      pass
...
def test_agent_behavior_with_harness(tmp_path):
  ...
  assert result1.summary()["runs_blocked"] == 0, f"Tool assertion failed: {result1.blocked_trace_ids}"

  order_policy = assert_span_order(["plan", "search_tool", "synthesize"])
  result2 = harness.simulate_policy(order_policy, traces)
  assert result2.summary()["runs_blocked"] == 0, f"Order assertion failed: {result2.blocked_trace_ids}"

  error_policy = assert_no_errors_in_trace()
  result3 = harness.simulate_policy(error_policy, traces)
  assert result3.summary()["runs_blocked"] == 0, f"Error assertion failed: {result3.blocked_trace_ids}"
...
def test_faulty_agent_fails_behavioral_test(tmp_path):
  ...
  assert summary["runs_blocked"] == 1, "The 'assert_no_errors' policy should have blocked this faulty trace."
...
def test_multiple_tool_calls(tmp_path):
  ...
  with tracer.span("workflow"):
    with tracer.span("search1", kind=SpanKind.CLIENT, attributes={"tool.name": "web_search"}):
      pass
    with tracer.span("search2", kind=SpanKind.CLIENT, attributes={"tool.name": "web_search"}):
      pass
    with tracer.span("calculator", kind=SpanKind.CLIENT, attributes={"tool.name": "calculator"}):
      pass
  ...
  assert result.summary()["runs_blocked"] == 0, "Should have found exactly 2 web_search calls"

  policy_wrong_count = assert_tool_was_called("web_search", times=3)
  result = harness.simulate_policy(policy_wrong_count, traces)
  assert result.summary()["runs_blocked"] == 1, "Should have blocked due to wrong count"
...
def test_agent_workflow_with_backtesting(tmp_path):
  ...
  for i in range(5):
    ...
    with tracer.span("workflow"):
      with tracer.span("analyze"):
        pass
      with tracer.span("search", kind=SpanKind.CLIENT, attributes={"tool.name": "search"}):
        pass
  ...
  assert summary["runs_blocked"] == 0, "All historical runs should pass the behavior test"

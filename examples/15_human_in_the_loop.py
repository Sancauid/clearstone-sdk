...
def run_financial_transaction(engine: PolicyEngine, context):
  ...
  try:
    ...
    print(f"⏸️ Transaction Paused by policy '{e.decision.metadata.get('policy_name', 'unknown')}'.")
    ...
    if intervention_client.wait_for_approval(intervention_id):
      ...
      approved_context = dataclasses.replace(context, metadata={**context.metadata, "is_approved": True})

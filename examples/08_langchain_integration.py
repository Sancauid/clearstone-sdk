...
from clearstone.integrations.langchain import PolicyCallbackHandler, PolicyViolationError
...
def require_approval_for_external_apis(context):
    ...
    if tool_name and "api" in tool_name.lower():
        ...
        if user_role != "admin":
            return BLOCK(f"External API access requires admin role. Current role: {user_role}")
...
def mock_langchain_agent_run():
    ...
    with context_scope(ctx):
        ...
        try:
            handler.on_tool_start(
                serialized={"name": "shell_executor"},
                input_str="rm -rf /"
            )

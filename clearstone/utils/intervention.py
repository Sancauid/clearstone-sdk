...
class InterventionClient:
  ...
  def wait_for_approval(self, intervention_id: str, prompt: str = None) -> bool:
    ...
    with _lock:
      ...
      if not intervention:
        print(f"Warning: Intervention ID '{intervention_id}' not found.", file=sys.stderr)
        return False

    prompt = prompt or f"Approve action for intervention '{intervention_id}'? (yes/no): "
    ...
    with _lock:
      _pending_interventions[intervention_id]['status'] = "approved" if approved else "rejected"
